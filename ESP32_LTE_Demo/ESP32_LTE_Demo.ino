...
void printMenu(void) {
  ...
  Serial.println(F("[Z] (UDP) Open UDP to PAE server"));
  Serial.println(F("[X] (UDP) Send message to PAE server"));
  Serial.println(F("[z] (UDP) Read received buffer"));
  Serial.println(F("[x] (UDP) Start active mode"));
  Serial.println(F("-------------------------------------"));
  Serial.println(F(""));
}
...
void loop() {
  ...
  switch (command) {
    ...
    case 'X': {
      // Write to PAE SERVER
      char string[80];
      Serial.println(F("Write message:"));
      uint16_t len = readline(string, 79);
      Serial.println(string);
      if (! fona.UDPsend(string, len)) Serial.println(F("Failed to send!"));
      break;
    }
